...
function changeQuickViewImage(newImage) {
  ...
  var imageCanvas = document.getElementById("productQuickViewModalDisplayImage");

  // Use the same src in the expanded image as the image being clicked on from the grid
  imageCanvas.src = newImage.src;
...
//-------------------------- Featured, New Arrivals, Top Sales Nav Tab -----------------------------

$('.featured-new-topsales-slide').slick({
  dots: false,
  infinite: false,
  speed: 300,
  slidesToShow: 4,
...
function toggleFeaturedNewTopSales(id) {
  var elementChildren = document.querySelectorAll('.featured-products-contents .tab-inner');
...
//-------------------------- Deals of The Day Start Progress -----------------------------
const progressBarsInner = [...document.querySelectorAll(".progress-bar-inner")];
progressBarsInner.forEach(pbi => {
  const percent = pbi.getAttribute('data-percent');
  pbi.style.width = `${percent}%`;
});



//-------------------------- Deals of The Day Start -----------------------------
$('.display-products-slide').slick({
  dots: false,
  infinite: false,
  speed: 300,
  slidesToShow: 6,
...
function toggleDisplayProductsContent(id, parentClass) {
  var elementChildren = document.querySelectorAll(`.${parentClass} .display-products-tab-inner`);
  elementChildren.forEach(child => {
    child.classList = `tab-inner display-products-tab-inner hidden`;
  });
  document.getElementById(id).classList = `tab-inner display-products-tab-inner display`;
}
